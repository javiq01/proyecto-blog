...
public class PublicationController implements GenericRestController<Publication, Long> {
	...
	public ResponseEntity<?> findById(Long id) {
		...
		if (publication == null) {
			return ResponseEntity.status(404).body(responseMessageService.getResponseMessage(MessageType.NO_ELEMENTS, "Publicacion con ID " + id + " No encontrada"));
		}
...
//		logger.info("Publication : " + publication);
	...
	public ResponseEntity<?> update(String uuid, @Valid Publication publication, BindingResult bindingResult) {
		...
		if (uuid == null) {
			return ResponseEntity.status(400).body(responseMessageService.getResponseMessage(MessageType.BAD_REQUEST, "credential [" + uuid + "] No encontrada"));
		}
		
		User user = loginService.getUser(uuid);
		if (user == null) {
			return ResponseEntity.status(409).body(responseMessageService.getResponseMessage(MessageType.VALIDATION_ERROR, "credential [" + uuid + "] No encontrada"));
		}

		publication.setUser(user);
//		logger.info("Publication update: " + publication);
		return save(publication,bindingResult);
	}
	...
	public ResponseEntity<?> delete(String uuid, @Valid Publication publication, BindingResult bindingResult) {
		...
		try {
			...
			logger.error(e);
			return ResponseEntity.status(404).body(responseMessageService.getResponseMessage(MessageType.NO_ELEMENTS, publication + " No encontrada"));
		}
		
		publicationRepository.delete(publication);
		user.getPublications().remove(publication);
		
		return ResponseEntity.ok(responseMessageService.getResponseMessage(MessageType.DELETE_ELEMENT, "Publicacion \"" + publication.getTitle() + "\" eliminada correctamente"));
	}
